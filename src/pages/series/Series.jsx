...
export const Series = () => {
  ...
  const {filteredFilms, search, savedMovies, handleBookmarkClick}= useContext(HeirWolfContext);
  const moviesFilter = filteredFilms.filter(film => film.category === "TV Series");
  ...
  return (
    ...
    <h1 className='titles recommend'>{search.length? `Found ${moviesFilter.length} results for "${search}"` : "TV Series"}</h1>
